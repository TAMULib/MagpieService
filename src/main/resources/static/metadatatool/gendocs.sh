...
if [ $# -eq 0 ]; then
	echo "Usage: gendocs [project name] [number of documents] [number of files/doc] [media type: txt, pdf, jpg, png, tif]"
else
	...
	while [[ $DOCCOUNT -lt $DOCS ]]; do
		...
		while [[ $FILECOUNT -lt $FILES ]]; do
			...
			if [[ " ${MEDIA_TYPES[@]} " =~ " png " ]] || [[ " ${MEDIA_TYPES[@]} " =~ " all " ]]; then
				cp ./sample-media/sample.png "projects/"$1"/"$1"_"$DOCCOUNT"/"$1"_"$DOCCOUNT"_"$FILECOUNT".png"
			fi
